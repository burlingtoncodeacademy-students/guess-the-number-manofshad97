...
async function rangeFunction(secretNum) {
  ...
  if (randNum < secretNum && rangeInput === "L" || randNum > secretNum && rangeInput === "H") {
...
async function mainGame() {
  ...
  maxNum = await ask("Choose a max range greater than 1: ")
  maxNum = parseInt(maxNum)
  randNum = Math.floor((maxNum + minNum) / 2)

  //If the max range inputted is not a number or less than or equal to 1, the user is prompted to enter a new range
  while(isNaN(maxNum) || maxNum <= 1){
    maxNum = await ask("Please enter an actual integer or a number above 1:")
    parseInt(maxNum)
  }
  ...
  // If the user's number is greater than the max, it means it is outside the range and is considered cheating.
  if(secretNumber > maxNum){
    console.log("I know your number is greater than the max. You're cheating! ")
    process.exit()
  }
  // If the user's number is less than the min, it means it is outside the range and is considered cheating.
  else if(secretNumber < minNum){
    console.log("I know your number is less than the min. You're cheating!")
    process.exit()
  }
  ...
  while (userInput !== "Y") {
    ...
    randNum = await rangeFunction(secretNumber)
...
async function chooseGame() {
  let userChoice = await ask("Would you rather play a game where I guess your number or a game where you guess mine? \nType '1' for the first option or '2' for the second: ")
  if (userChoice === "1") {
    mainGame()
  }
